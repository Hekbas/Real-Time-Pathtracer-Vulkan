...
void main() {
    ...
    if (face.diffuseTextureID != -1) {
        // Convert from sRGB to linear space for rendering
        vec3 texColor = texture(nonuniformEXT(diffuseTextures[nonuniformEXT(face.diffuseTextureID)]), texCoord).rgb;
        diffuseColor = pow(texColor, vec3(2.2)); // sRGB to linear
    }
    ...
    payload.albedo   = diffuseColor; // Store albedo for GBuffer
    payload.done     = false;
}